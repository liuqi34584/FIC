...
def padding_img(image, up, down, left, right):

    Reflect_img = cv2.copyMakeBorder(image, up, down, left, right, cv2.BORDER_REFLECT)  # 反射法，在图像边缘进行镜像反射

    return Reflect_img


# 仿射变换注释框,传入原图，原box,旋转角度，返回新box
def padding_box(image, box, up, down, left, right):
    ...
    for (name, x, y, x_max, y_max) in box:
        ...
        padded_image = padding_img(padded_image, up, down, left, right)
